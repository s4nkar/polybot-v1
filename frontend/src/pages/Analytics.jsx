  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [a, t] = await Promise.all([
        getAnalytics(mode),
        getAllTrades(PAGE_SIZE, page * PAGE_SIZE, mode),
      ]);
      setAnalytics(a);
      setTrades(t.trades || []);
      setTotal(t.total || 0);
                      ...
                      {trades.map((t, i) => (
                        ...
                        >
                          ...
                          <td className={`px-4 py-3 text-right font-mono font-semibold ${
                            t.pnl_usd > 0 ? "text-emerald-400" : t.pnl_usd < 0 ? "text-red-400" : "text-surface-400"
                          }`}>
                            {t.pnl_usd != null ? `${t.pnl_usd >= 0 ? "+" : ""}$${Number(t.pnl_usd).toFixed(4)}` : "—"}
                          </td>
                          <td className={`px-4 py-3 text-right font-mono font-semibold ${
                            t.pnl_pct > 0 ? "text-emerald-400" : t.pnl_pct < 0 ? "text-red-400" : "text-surface-400"
                          }`}>
